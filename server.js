...
app.get('/', (req, res) => {
  res.json({ status: 'BRT Tracker ready', version: '1.0' });
});
...
app.post('/track', async (req, res) => {
  ...
  try {
    ...
    await page.goto('https://services.brt.it/it/tracking', { waitUntil: 'networkidle2', timeout: 30000 });
    await delay(2000);
    ...
    try {
      await page.evaluate(() => {
        for (const b of document.querySelectorAll('button, a')) {
          ...
          if (t.includes('rifiuta') || t.includes('reject') || t === 'accetta selezionati') { b.click(); return; }
        }
      });
      await delay(1000);
    } catch (e) {}
    ...
    // Step 4: Wait for mybrt.it
    try { await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 30000 }); } catch (e) {}
    await delay(3000);
    if (page.url().includes('services.brt.it')) await delay(5000);
    ...
    const result = await page.evaluate(() => {
      const d = { hasFullDetails: false, status: 'unknown', pickupPointName: '', pickupPointAddress: '', brtCode: '', events: [], pageType: '' };

      d.hasFullDetails = !!document.querySelector('.content-item-track');
      d.pageType = d.hasFullDetails ? 'FULL_DETAILS' : (document.querySelector('#verificationCode') ? 'DATA_PROTECTION' : 'OTHER');
          ...
          if (div.querySelector('.delivery-address-icon.location')) {
            for (const p of div.querySelectorAll('p')) {
              if (!p.classList.contains('delivery-address-icon') && p.textContent.trim()) {
                d.pickupPointAddress = p.textContent.trim();
                break;
              }
            }
          }
